...
void PlayerAAA::DrawTankRing() {
    ...
    //m_tstatus.data.ringData 0 やや短距離  矢豆　⻊巨 离隹
    //m_tstatus.data.ringData 1 短距離  
    //m_tstatus.data.ringData 2 中距離  
    //m_tstatus.data.ringData 3 長距離  
    //m_tstatus.data.ringData 4 最長距離
    RECT rc1 = { startX + 24 + 50, startY + 2, WIDTH, HEIGHT };
    if (m_tstatus.data.ringData == 0) {
        
        font->DrawText(nullptr, L"短　⻊巨 离隹", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));
  
    }
    else if (m_tstatus.data.ringData ==1){

        font->DrawText(nullptr, L"矢豆 ⻊巨 离隹", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));

    }

    else if (m_tstatus.data.ringData ==2) {

        font->DrawText(nullptr, L"中　⻊巨 离隹", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));

    }

    else if (m_tstatus.data.ringData == 3) {

        font->DrawText(nullptr, L"長　⻊巨 离隹", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));

    }
    else {

        font->DrawText(nullptr, L"最　長　⻊巨 离隹（MAX）", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));
    }
}
...
void PlayerAAA::DrawTankSpeed() {
    ...
    if (m_tstatus.data.speedData == 0) {
        ...
        font->DrawText(nullptr, L"遅　い", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));
        ...
        font->DrawText(nullptr, L"普　通", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));
    ...
    {
        font->DrawText(nullptr, L"速　い（MAX）", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));
    }
}
...
void PlayerAAA::DrawBulletMark() {
    ...
    if (m_tstatus.data.bulletData == 0) {
        ...
        font->DrawText(nullptr, L"壱　連　発", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));

    }
    else if (m_tstatus.data.bulletData == 1) {

        font->DrawText(nullptr, L"弐　連　発", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));

    }
    else if (m_tstatus.data.bulletData == 2) {

        font->DrawText(nullptr, L"参　連　発", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));

    }
    else  {

        font->DrawText(nullptr, L"肆　連　発 （MAX）", -1, &rc1, DT_LEFT | DT_TOP, D3DCOLOR_XRGB(255, 178, 0));
